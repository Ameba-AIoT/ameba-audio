...
void ameba_audio_dump_codec_regs(void)
{
	...
	for (uint32_t i = 0; i < MAX_AD_NUM; i++) {
		...
		HAL_AUDIO_INFO("CODEC_ADC_%lu_CONTROL_0:%lx", i, tmp);
		tmp = audio_base->CODEC_ADC_CH_CTRL[i].CODEC_ADC_x_CONTROL_1;
		HAL_AUDIO_INFO("CODEC_ADC_%lu_CONTROL_1:%lx", i, tmp);
	}

	tmp = audio_base->CODEC_DAC_L_CONTROL_0;
	HAL_AUDIO_INFO("CODEC_DAC_L_CONTROL_0:%lx", tmp);
	tmp = audio_base->CODEC_DAC_L_CONTROL_1;
	HAL_AUDIO_INFO("CODEC_DAC_L_CONTROL_1:%lx", tmp);
	tmp = audio_base->CODEC_DAC_L_CONTROL_2;
	HAL_AUDIO_INFO("CODEC_DAC_L_CONTROL_2:%lx", tmp);
	tmp = audio_base->CODEC_DAC_R_CONTROL_0;
	HAL_AUDIO_INFO("CODEC_DAC_R_CONTROL_0:%lx", tmp);
	tmp = audio_base->CODEC_DAC_R_CONTROL_1;
	HAL_AUDIO_INFO("CODEC_DAC_R_CONTROL_1:%lx", tmp);
	tmp = audio_base->CODEC_DAC_R_CONTROL_2;
	HAL_AUDIO_INFO("CODEC_DAC_R_CONTROL_2:%lx", tmp);
