...
static AACDecInfo obj_aacDecInfo;
static PSInfoBase obj_psInfoBase;
#endif
HAACDecoder AACInitDecoder(void)
{
#if defined(STATIC_RESOURCE)
	AACDecInfo *aacDecInfo = &obj_aacDecInfo;
	memset(aacDecInfo, 0, sizeof(AACDecInfo));
	aacDecInfo->psInfoBase = &obj_psInfoBase;
	memset(aacDecInfo->psInfoBase, 0, sizeof(PSInfoBase));
#else
#ifdef USE_SRAM
	AACDecInfo *aacDecInfo = (AACDecInfo *)RtSRAMMalloc(sizeof(AACDecInfo));
#else
...
#ifdef USE_SRAM
	aacDecInfo->psInfoBase = (PSInfoBase *)RtSRAMMalloc(sizeof(PSInfoBase));
#else
	aacDecInfo->psInfoBase = (PSInfoBase *)malloc(sizeof(PSInfoBase));
#endif
...
/**************************************************************************************
 * Function:    AACDeInitDecoder
 *
 * Description: free resource allocated by AACInitDecoder
 *
 * Inputs:      HAACDecoder
 *
 * Outputs:     none
...
#ifdef USE_SRAM
	if (aacDecInfo->psInfoBase) {
		RtSRAMFree(aacDecInfo->psInfoBase);
	}
	if (aacDecInfo) {
		RtSRAMFree(aacDecInfo);
	}
#else
	if (aacDecInfo->psInfoBase) {
...
int AACFindSyncWord(unsigned char *buf, int nBytes)
{
	...
	for (i = 0; i < nBytes - 1; i++) {
		if ((buf[i + 0] & SYNCWORDH) == SYNCWORDH && (buf[i + 1] & SYNCWORDL) == SYNCWORDL && (buf[i + 1] & 0x06) == 0) {
			return i;
		}
	}
...
#endif
	unsigned char *inbuf_backup = *inbuf;
	int bytesLeft_backup = *bytesLeft;
